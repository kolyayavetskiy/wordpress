...
switch ( $wp_list_table->current_action() ) {
	...
	case 'promote':
		...
		foreach ( $userids as $id ) {
			...
			if ( $id == $currentUser->ID && ! $wp_roles->role_objects[ $role ]->has_cap( 'promote_users' )
	...
	case 'delete':
		...
		if ( in_array( $currentUser->ID, $userids, true ) ) {
			$userids = array_diff( $userids, array( $currentUser->ID ) );
		}
...
<ul>
		...
		<ul style="list-style:none;">
			 ...
			 * @param int[]   $userids      Array of IDs for users being deleted.
			 */
			do_action( 'delete_user_form', $currentUser, $userids );
			?>
